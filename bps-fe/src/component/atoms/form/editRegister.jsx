...
const EditRegisterForm = ({ data, setShowModal, SetChange, typePage }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const editUser = async (dataEdited) => {
    ...
    try {
      await axios
        .put(`${endpoint}/users/${data.id}`, dataEdited)
        .then(() => {
          setLoading(false);
          Swal.fire({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <select
          {...register("roleId", { required: true, maxLength: 200 })}
      ...
      >
        ...
        <select
          {...register("teamId", { required: true, maxLength: 200 })}
          className="text-[#737373] border border-gray-300 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full py-3 px-1 "
        >
          <option value="" selected>
            Pilih Team
