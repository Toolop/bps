...
import { GetScheduleUsersHooks } from "../../../hooks/scheduleUsers/getScheduleUsersHooks";

export const FormPersonilAssignScheduleTeam = ({
  data,
  setShowModal,
  setChange,
}) => {
  ...
  const { getScheduleUsers, SetChange } = GetScheduleUsersHooks(data.id);
  ...
  const deleteAssign = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios
            .delete(`${endpoint}/schedule-users/${id}`, {
              headers: {
                Authorization: token,
              },
            })
    ...
    <div className="w-full h-full min-h-[400px] flex flex-col gap-5">
      <div className="flex items-center justify-center gap-3">
        <div className="flex w-full flex-col relative">
          ...
          >
            <p className="w-full ml-3">{name == "" ? "Pilih Nama" : name}</p>
      ...
      <div className="flex flex-wrap border-solid border-2 border-gray-200 rounded p-5 h-100">
        {getScheduleUsers
          ? getScheduleUsers.map((item, index) => (
              <span
                key={index}
